...
async function seed() {
    ...
    const [newestRow] = await db_1.db
        .select({ checkedAt: schema_1.checkResults.checkedAt })
    ...
    if (newestRow) {
        ...
        await db_1.db.delete(schema_1.checkResults);
    ...
    let monitorRows = await db_1.db.select({ id: schema_1.monitors.id }).from(schema_1.monitors);
    if (monitorRows.length > 0) {
        ...
        for (let minAgo = WINDOW_MINUTES - 1; minAgo >= 0; minAgo--) {
            ...
            let scenario;
            if (minFromStart >= 26 && minFromStart <= 28)
                scenario = 'spike';
            else if (minFromStart >= 21 && minFromStart <= 25)
                scenario = 'degraded';
            else if (minFromStart >= 29)
                scenario = 'recovery';
            else
                scenario = 'stable';
        ...
        for (let offset = 0; offset < rows.length; offset += BATCH) {
            await db_1.db.insert(schema_1.checkResults).values(rows.slice(offset, offset + BATCH));
        }
